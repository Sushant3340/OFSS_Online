...
public partial class StudentLogin_PrintCAF_SpotAdmission_Jun : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            if (Session["StudID"] != null)
            {
                ...
                else
                {
                    if (FromDate < 0)
                    {
                        litMessage.Text = "Dateline to print CAF for Spot admission is not started yet...";
                    }
                    if (ToDate < 0)
                    {
                        litMessage.Text = " Dateline to print CAF for Spot admission is closed...";
                    }
    ...
    protected void fillGeneralInformation(string strId)
    {
        ...
        try
        {
            ...
            obj2.vch_UniqueRefNo = Session["StudID"].ToString();
                ...
                strCurrSession = "20" + Convert.ToInt32(strId.Substring(0, 2).ToString());
                hdnCurrSession.Value = strCurrSession;
                if (Convert.ToInt32(strId.Substring(0, 2).ToString()) < Convert.ToInt32(strCurrFYear.Substring(strCurrFYear.Length - 2, 2).ToString()))
                {
                    list = ccobj.FillCAF_PrevYr(obj);
                    list1 = ccobj.FillCAF_PrevYr(obj1);
                    listoption = ccobj.FillCAF_PrevYr(obj2);
                }
                else
                {
            ...
            if (list.Count > 0)
            {
                ...
                else
                {
                    ...
                    //--------------Check for EWS for the year 2018
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(list[0].vchImageName)))
                {
                    ...
                    imgPhoto.ImageUrl = imgPhoto.ImageUrl = "~/DownloadImage.ashx?clsid=1&id=" + list[0].CorBlockId.ToString() + "/" + list[0].vchImageName + "&YearId=" + strCurrSession;
                }
                else
                {
                ...
                lblLastModify.Text = list[0].StrName;

                if (Convert.ToInt32(hdnCurrSession.Value) == 2018)
                {
                    trEWS.Visible = false;
                    tdlblWBC.Style.Add("display", "");
                    tdtxtWBC.Style.Add("display", "");
                }
                else
                {
                    trEWS.Visible = true;
                    tdlblWBC.Style.Add("display", "none");
                    tdtxtWBC.Style.Add("display", "none");
                }
            }
            if (!string.IsNullOrEmpty(Convert.ToString(ConfigurationManager.AppSettings["AppInstance"])))
            {
                if (string.Equals(ConfigurationManager.AppSettings["AppInstance"].ToString(), "staging", StringComparison.OrdinalIgnoreCase))
                {
                    divStaging.Visible = true;
                }
                else
                {
