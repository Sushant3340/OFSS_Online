...
public partial class StudentLogin_PrintTransferJrCAF : System.Web.UI.Page
{
    int intOptionCnt;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Session["TransferStudSts"].ToString() == "1")
            {
        ...
        try
        {
            ...
            if (list.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(list[0].BoardId)))
                {
                    ...
                    {
                        if ((list[0].BoardId == 46 || list[0].BoardId == 47) && list[0].int_YearOfPassing == 2024)
                        {
