...
public partial class ONLINE_CAF_ConfirmCAFJrClg : System.Web.UI.Page
{
    ...
    protected void viewAppDetails()
    {
        ...
        if (Convert.ToInt32(HttpContext.Current.Items["QuotaType"]) == 1)
        {
            lblQuotaType.Text = "Minority";
        }
        else if (Convert.ToInt32(HttpContext.Current.Items["QuotaType"]) == 2)
        {
            lblQuotaType.Text = "Coaching Quota";
        }
        else
        {
            lblQuotaType.Text = "Other Quota";
        }
    ...
    protected void sendTransferData()
    {
        ...
        try
        {
            ...
            //if ( Convert.ToInt32(strBoard.Value.Split('~')[0]) == 47 && Convert.ToInt32(HttpContext.Current.Items["YOP"]) == 2019)
            //{
            ...
            if (lblQuotaType.Text == "Minority")
            {
                HttpContext.Current.Items["QuotaType"] = "1";
            }
            else if (lblQuotaType.Text == "Coaching Quota")
            {
        ...
        {
            IsException = true;
            Util.LogError(ex, "ConfirmCAFJrSpotClg");
        }
        finally
        {
        ...
        if (!IsException)
        {
            Server.Transfer("JrCAFFormSpotClg.aspx", true);
        }
    }
    #endregion
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(hidPhoto.Value) && ViewState["ByteStream"] != null)
        {
            AddCAFData("Q"); //modified by Ritika lath on 3rd July 2019 for quota admission
        }
        else
        {
    ...
    protected void AddCAFData(string strAction)
    {
        ...
        try
        {
            ...
            ObjPAS.vch_Grade = "0";
            //}
            if ((ObjPAS.BoardId == 46 & Convert.ToInt32(lblYOE.Text) >= 2010 & Convert.ToInt32(lblYOE.Text) < 2018) | (ObjPAS.BoardId == 103 & Convert.ToInt32(lblYOE.Text) >= 2012))
            {
            ...
            if (!string.IsNullOrEmpty(hidPhoto.Value) && ViewState["ByteStream"] != null)
            {
                byte[] bytes = (byte[])ViewState["ByteStream"];
                ...
                pth = pth + "\\" + strImageName + ".jpg";
                reSize.Save(pth, ImageFormat.Jpeg);
                reSize.Dispose();
            }
        ...
        {
            Util.LogError(ex, "ConfirmCAFJrSpotClg" + "::Board_" + strBoard.Value.Split('~')[0] + "YOP_" + lblYOE.Text + "Roll_" + lblRoll.Text + "Rcode_" + lblRollCode.Text + "::" + lblTotalMark.Text + "," + lblMaxMark.Text);
        ...
        else if (intRetVal == 3)
        {
            ...
            string strMsg = "आपने पहले हीं  Mobile Number: " + strMobno + " से OFSS के माध्यम से आवेदन फॉर्म भर दिया है इसलिए आप दुबारा नया फॉर्म नहीं भर पा रहे हैं |";
        ...
        {
            string str = "Only boys candidates are allowed in this college";
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "alert", "alert('" + str + "');", true);
        }
        else if (intRetVal == 7)
        {
            string[] strAdmissionDtl = strResult.Trim().Split('~');
            string str = string.Format("This applicant has already taken admission in {0}, stream - {1} under {2} on {3}. Please ask the applicant to cancel the admission to the admitted college and then only he can be admitted in this college.", strAdmissionDtl[1], strAdmissionDtl[2], strAdmissionDtl[3], strAdmissionDtl[4]);
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "alert", "alert('" + str + "');", true);
        }
    }
        ...
        try
        {
            if (lblemail.Text.Trim() != "")
            {
                string mailBody =
                  ...
                  "नए OFSS CAF के लिए OTP " + strDynamicPwd + " लागू है, कृपया  2 मिनट के भीतर यह ओटीपी उपयोग करें और उसके बाद भुगतान के लिए आगे बढ़ें";
    ...
    {
        string strURL = "QuotaSuccess.aspx?";
