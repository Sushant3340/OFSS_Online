...
public partial class ONLINE_CAF_JrCAFFormSpotClg : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!(Request.QueryString["open"] != null && Request.QueryString["open"] == "1412"))
            {
            ...
            hidDistrictID.Value = Request.QueryString["did"];
            hidCollegeID.Value = Request.QueryString["cid"];
            if (!IsPostBack)
            {
                btnSave.Text = "यहाँ क्लिक करें | Please click here";

                /*add code to fill the gender and set the category based on the collegeid*/
                FillGenderAndCatgeory();

                /*add code to fill the stream based on the collegeid*/
                //  getStream(Request.QueryString["cid"]);
        ...
        {
            Util.LogError(ex, "JrCAFFormspotClg");
        }
    }
    ...
    protected void viewPreviousData()
    {
        ...
        try
        {
            ...
            if (HttpContext.Current.Items["QuotaType"].ToString() == "1")
            {
                rbtnMinority.Checked = true;
            }
            else
            {
                rbtnMinority.Checked = false;
            }
            if (HttpContext.Current.Items["QuotaType"].ToString() == "2")
            {
                rbtnCoaching.Checked = true;
            }
            else
            {
                rbtnCoaching.Checked = false;
            }
            if (HttpContext.Current.Items["QuotaType"].ToString() == "3")
            {
                rbtnOtherQuota.Checked = true;
            }
            else
            {
                rbtnOtherQuota.Checked = false;
            }
        }
    ...
    protected void clientSideMethod()
    {
        ...
        //this.ddlCollege.Attributes.Add("onchange", "fillStream(" + ddlCollege.ClientID + ");");
    ...
    protected void fillCollege()
    {
        ...
        DataView dvSource = default(DataView);
        string pth = ConfigurationManager.AppSettings["StrPath"].ToString();
        ddlDataSource.ReadXml(pth + "SAMS\\MasterXML/M_COLLEGE_QUOTA.xml");
        //ddlDataSource.ReadXml(Server.MapPath("../MasterXML/M_COLLEGE_QUOTA.xml"));
        dvSource = ddlDataSource.Tables[0].DefaultView;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        // if (txtMaxMark.Text !="" && txtTotMark.Text != "")
        // {
        //     if (Convert.ToInt32(txtTotMark.Text) > Convert.ToInt32(txtMaxMark.Text))
        //     {
        //         str = "Total Mark secured cannot be greater than Maximum Mark";
        //     }
        // }
    ...
    protected void sendTransferData()
    {
        ...
        try
        {
            ...
            {
                HttpContext.Current.Items["Photo"] = this.hdnImgAppl.Value;
            }
            ...
            if(rbtnMinority.Checked == true)
            {
                HttpContext.Current.Items["QuotaType"] = 1;
            }
            else if (rbtnCoaching.Checked == true)
            {
                HttpContext.Current.Items["QuotaType"] = 2;
            }
            else
            {
                HttpContext.Current.Items["QuotaType"] = 3;
            }
        ...
        if (!IsException)
        {
            Server.Transfer("ConfirmCAFJrSpotClg.aspx", true);
        }
    ...
    #region fillStream
    [WebMethod()]
    public static dynamic FillStream(int intCollegeID, int quotaType)
    {
        CAFDAL ccobjcaf = new CAFDAL();
    ...
    [WebMethod()]
    public static dynamic fillDistWiseColg(int intDistId, int intCType, int intGender, int restype, int CollegeId)
    {
        dynamic qry = null;
        try
        {
    ...
    [WebMethod()]
    public static dynamic LoadDistrict(int intDistrictID)
    {
        System.Xml.Linq.XDocument xdoc = default(System.Xml.Linq.XDocument);
        string pth = ConfigurationManager.AppSettings["StrPath"].ToString();
        xdoc = XDocument.Load(pth + "sams/MasterXML/M_DISTRICT.xml");
    ...
    /// <summary>
    /// Function to set gender and category based on the college 
    /// Added by Ritika lath on 26th June 2019
    /// </summary>
    private void FillGenderAndCatgeory()
    {
        ddlGender.Items.Clear();
        int intCid = Convert.ToInt32(Request.QueryString["cid"]);
        System.Xml.Linq.XDocument xdoc = default(System.Xml.Linq.XDocument);
        string pth = ConfigurationManager.AppSettings["StrPath"].ToString();
        xdoc = XDocument.Load(pth + "sams/MasterXML/M_COLLEGE_QUOTA.xml");
        ...
        qry = from p in xdoc.Descendants("NewDataSet").Elements("Table")
              where Convert.ToInt32(p.Element("CID").Value) == intCid
              select new
              {
                  gender = p.Element("WomenStatus").Value
                  ,
                  resType = p.Element("ResStatus").Value
              };

        foreach (var r in qry)
        {

            int intgender = Convert.ToInt32(r.gender);
            if (intgender == 0)
            {
            ...
            else if (intgender == 1)
            {
                ...
                ddlGender.Items.Add(new ListItem("TRANSGENDER", "3"));
            }
            else if (intgender == 2)
            {
                ddlGender.Items.Add(new ListItem("FEMALE", "2"));
            ...
            int intResType = Convert.ToInt32(r.resType);

            hdnResCategory.Value = intResType.ToString();
            if (intResType == 2)
            {
                rbtSC.Checked = true;
                rbtGeneral.Enabled = false;
                rbtST.Enabled = false;
                rbtnOBC.Enabled = false;
