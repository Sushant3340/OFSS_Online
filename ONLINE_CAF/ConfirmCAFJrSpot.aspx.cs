...
//' Author                 :  
//' Created on             :  
//' File Name              :  ConfirmCAFSpot.aspx.vb
...
public partial class ConfirmCAFJrSpot : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            IsException = false;
            Util.LogError(ex, "ConfirmCAFJrSpot");
            //Server.Transfer("Error.aspx", False)
        }

        if (!IsException)
        {
            Server.Transfer("JrCAFFormSpot.aspx", true);
        }
    ...
    protected void viewAppDetails()
    {
        ...
        if (HttpContext.Current.Items["Photo"] != null)
        {
            ...
            imgPhoto.ImageUrl = pth + hidPhoto.Value; //Attaching Photo to Image Field
    ...
    protected void sendTransferData()
    {
        ...
        if (!IsException)
        {
            //HttpContext.Current.RewritePath("ConfirmCAF.aspx", true);
            Server.Transfer("JrCAFFormSpot.aspx", true);
        }
    ...
    protected void AddCAFData(string strAction)
    {
        ...
        try
        {
            ...
            ObjPAS.CorVillageName = lblOtherBoard.Text.Trim().ToString();
            if (lblExamType.Text == "Annual")
            {
                ObjPAS.ExamType = "1";
            }
            ...
            //modified by Ritika lath on 09092020
            ...
            //    if (bytes == null)
            //    {
            //        string str = "Please Upload your Photo again";
            //        ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "alert", "alert('" + str + "');", true);
            //        return;
            //    }
            //}
                ...
                strResult = ccobjcaf.AddCAFData_Spot(ObjPAS);
        ...
        {
            isexception = true;
            Util.LogError(ex, "ConfirmCAFJrSpot" + "::Board_" + strBoard.Value.Split('~')[0] + "YOP_" + lblYOE.Text + "Roll_" + lblRoll.Text + "Rcode_" + lblRollCode.Text + "::" + lblTotalMark.Text + "," + lblMaxMark.Text);
