...
function addRow() {
    ...
    /*added by Ritika lath to freeze all the above 6 values*/
    if ($("#ddlBoard").val() == "109") {
        $("#ddlBoard").css("display", "none");
        $("#rbtnAnnual, #rbtnSuppl").css("display", "none");
        $("#rbtnAnnual, #rbtnSuppl").next("label").css("display", "none");
        $("#ddlYOP").css("display", "none");
        $("#ddlDay").css("display", "none");
        $("#ddlMonth").css("display", "none");
        $("#ddlYear").css("display", "none");
        $("#txtRollCode").css("display", "none");
        $("#txtBoardRoll").css("display", "none");

        $("#lblBoardVal").css("display", "").text($("#ddlBoard option:selected").html());
        $("#lblYopVal").css("display", "").text($("#ddlYOP option:selected").html());
        $("#lblDOBVal").css("display", "").text($("#ddlDay option:selected").html() + "-" + $("#ddlMonth option:selected").html() + "-" + $("#ddlYear option:selected").html());
        if ($("#rbtnAnnual").prop("checked")) {
            $("#lblExamtypeVal").css("display", "").text("Annual");
        }
        else {
            $("#lblExamtypeVal").css("display", "").text("Compartmental");
        }
        $("#lblRollCodeVal").css("display", "").text($("#txtRollCode").val());
        $("#lblRollNoVal").css("display", "").text($("#txtBoardRoll").val());
    }
...
function remove(id) {
    ...
    if (confirm('Are you sure you want to delete this option ?')) {
        ...
        for (i = 1; i < rowsLen; i++) {
            ...
            //alert(Rows[i].getElementsByTagName("TD")[6].innerHTML);
        ...
        if (rowsLen == 1) {
            ...
            //aded by Ritika lath on 13th May to show and hide proper message for BSEB students
            $("#trbsebdisable").css("display", "none");
            $("#lblBsebdisable").css("display", "none");
    ...
    if (document.getElementById('tableOption').getElementsByTagName("TR").length <= 1) {
        ...
        /*added by Ritika lath to freeze all the above 6 values*/
        if ($("#ddlBoard").val() == "109") {
            $("#ddlBoard").css("display", "");
            $("#rbtnAnnual, #rbtnSuppl").css("display", "");
            $("#rbtnAnnual, #rbtnSuppl").next("label").css("display", "");
            $("#rbtnAnnual, #rbtnSuppl").parent("span").css("display", "");
            $("#ddlYOP").css("display", "");
            $("#ddlDay").css("display", "");
            $("#ddlMonth").css("display", "");
            $("#ddlYear").css("display", "");
            $("#txtRollCode").css("display", "");
            $("#txtBoardRoll").css("display", "");

            $("#lblBoardVal").css("display", "none").html('');
            $("#lblYopVal").css("display", "none").html('');
            $("#lblDOBVal").css("display", "none").html('');
            $("#lblExamtypeVal").css("display", "none").html('');
            $("#lblRollCodeVal").css("display", "none").html('');
            $("#lblRollNoVal").css("display", "none").html('');
        }
    }
        ...
        if (totRow > 1) {
            ...
            if (parseInt(colCntr) > 0) //&& (parseInt(stCntr) > 0))
            {
                ...
                if (optLen != id) {
                    ...
                    var board = $("#dlBoard").val();
                    if (board != null && board != undefined && board != '' && board != '0') {
                        if (ctlId == 2) { //this is the first row being added
                            $("#trbsebdisable").show();
                            $("#lblBsebdisable").show();
                        }
                    }
...
function ValidateForm() {
    ...
    //change made on 18th March 2020 to add year 2020
    if (((board == 46) && (yop >= 2010) && (yop <= 2017))  || ((board == 103) && (yop >= 2012))) { //for cbse and year 2010 to 2017
    ...
    else if ((board == 116) && (yop >= 2010)) { //for kerela board from 2010
        ...
        // if (Tot == 0 && (board != 46) && (board != 47)) { // added by Ritika lath on 22nd April to bypoass this validation for cbse and ICSE Students as they are allowed to enter 0
        if (Tot == 0) {
        ...
        if (Tot >= Max) {
            alert('Total Mark obtained cannot be greater than or equal to Total Full Marks');
            document.getElementById('txtTotMark').focus();
            return false;
        }
        ...
        if (perc >= 100.00) {
            alert('Please enter valid total secured marks');
            document.getElementById('txtTotMark').focus();
            return false;
        }
    ...
    //if ((document.getElementById('tableOption').getElementsByTagName("TR").length < 11) && (cIndex == 0) && (sIndex == 0) && (comp1Index == 0) && (comp2Index == 0) && (comp3Index == 0) && (e1Index == 0) && (e2Index == 0) && (e3Index == 0) && (f1Index == 0)) {
    //    alert('Please select minimum 10 Option details.\n कृपया न्यूनतम 10 विकल्प चुने |');
    //    document.getElementById('ddlCollege').focus();
    //    return false;
    //}

    if ((cIndex != 0) && (sIndex != 0)) {
        if ((document.getElementById('tableOption').getElementsByTagName("TR").length < 10)) {
            alert('Please select minimum 10 Option details.\n कृपया न्यूनतम 10 विकल्प चुने |');
            document.getElementById('ddlCollege').focus();
            return true;
        }
...
function BoardMarkCheck() {
    ...
    if (inTotal > Tot) {
        alert('The sum of your individual marks cannot be greater than the Total Mark Obtained');
        document.getElementById('txtTotMark').focus();
        return false;
    }
    if (Max == 0) {
    ...
    for (k = 0; k < Arr.length; k++) {
        ...
        for (var i = 0; i < str1.length; i++) {
            ...
            if ((ch == "`") || (ch == "'") || (ch == ">") || (ch == "<") || (ch == "!") || (ch == "^") || (ch == "%") || (ch == "?") || (ch == "~") || (ch == "!") || (ch == "@") || (ch == "#") || (ch == "$") || (ch == "&") || (ch == "*") || (ch == "(") || (ch == ")") || (ch == "_") || (ch == "+") || (ch == "|") || (ch == "[") || (ch == "]") || (ch == "{") || ch == "}" || (ch == ":") || (ch == ";") || (ch == '=') || (ch == '"')) {
                alert(' Special characters key except Space, Hyphen, Slash and Comma are not allowed./ स्पेस, हाइफ़न, स्लैश और कोमा को छोड़कर विशेष वर्ण कुंजी की अनुमति नहीं है।');
                document.getElementById(Arr[k]).value = '';
                document.getElementById(Arr[k]).focus();
                return false;
            }
...
function clearRollNumber() {
    //added by ritika lath on 02 april 2020 as when option is added gender is disabled and gender has to be autopopulated in many cvases
    // if (!(document.getElementById('tableOption').getElementsByTagName("TR").length > 1)) {
    $("#txtBoardRoll").val('');
    $("#txtRollCode").val('');
    $("#ddlDay").val('0');
...
function ExamType() {
   ...
   // if ($('#ddlYOP').val() == 2023) { //modified to change academic year

   //     document.getElementById('rbtnAnnual').checked = true;
   // }

   //// ---------------commented on 28th June 2022 to allow compartmental students passed in the same year
   // if ($('#ddlYOP').val() == 2023) { //modified to change academic year
...
function loadColleges() {
    ...
    if (inVal != '0') {
        ...
        else if (document.getElementById('rbtST').checked == true) { //modified by Ritika lath to get correct ST Colleges, again reverted to 4 on 30th July 
            restype = 4;
        }
        else if (document.getElementById('rbtnOBC').checked == true) {
            restype = 3;
...
function OptionValidation(ctrlName) {
    ...
    if (document.getElementById('tableOption').getElementsByTagName("TR").length > 1) {
        ...
        if (ctrlName == 'ddlGender') {
            ...
            msg = 'Delete all Selected College options and then change your gender. After that you will have to select college options again';
        }
        var category = $('#hidCategory').val();
        if (ctrlName == 'cast') {
            msg = 'Delete all Selected College options and then change your reservation details. After that you will have to select college options again';

        }

        // $("#rbtGeneral, #rbtSC,#rbtST,#rbtnOBC, #rbtOther").removeAttr("checked");
        if (category == '1') {
...
function BoardMark() {
    ...
    if (inVal == 109) {
        ...
        if (rollcd != null && rollcd != undefined && rollcd != '' && rollcd.length != 5) {
            isValid = false;
            strMessage = "Invalid roll code. Roll Code for BSEB should be of length - 5";
            controlid = 'txtRollCode';
        }
        else if (rollcd != null && rollcd != undefined && rollcd != '' && (!regexNumeric.test(rollcd))) {
            isValid = false;
            strMessage = "Only number are allowed in Roll Code for BSE Board!!";
            controlid = 'txtRollCode';
        }
        else if (roll != null && roll != undefined && roll != '' && (roll.length < 4 || roll.length > 7)) {
            isValid = false;
            strMessage = "Invalid roll number. Roll number length for BSEB should be between 4 and 7";
            controlid = 'txtBoardRoll';
        }
        else if (roll != null && roll != undefined && roll != '' && (!regexNumeric.test(roll))) {
            isValid = false;
            strMessage = "Only number are allowed in Roll Number for BSE Board!!";
            controlid = 'txtBoardRoll';
        }
                    ...
                    if (lstdtl.length > 0) {
                        ...
                        if (yr >= 2010) { //data is present in db check for grade 
                            ...
                            if (grade == 1) { //1 means pass , logic added by Ritika lath on 08-04-2019
                                ...
                                //}
                                //$('#hdnUniqueId').val(lstdtl[0].pStrName);
...
//    var regexNumeric = /^[0-9][0-9]*$/;
//    if (yr == 2024 && inVal == 109) {
//        if (UniqueId != null && UniqueId != undefined && UniqueId != '' && (!regexNumeric.test(UniqueId))) {
//            isValid = false;
//            strMessage = "Only number are allowed in Applicant's unique id for BSE Board!!";
//            controlid = 'txtUniqueId';
//        }
//    }
//}

function VerifyMaximumMarks() {
    var maxMark = $("#txtMaxMark").val();
    var totalmark = $("#txtTotMark").val();
    if (maxMark != null && maxMark != undefined && maxMark != '' && totalmark != null && totalmark != undefined && totalmark != '') {
        if (parseInt(maxMark) < parseInt(totalmark)) {
            alert('Total Mark obtained cannot be greater than Total Full Marks');
            document.getElementById('txtTotMark').focus();
            return false;
        }
...
function fillMaximumMark() {
    ...
    //$.ajax({
    //    type: 'POST',
    //    url: 'JrCAFForm.aspx/fillMaximumMark',
    //    data: "{'intBoardId':" + boardId + ",'intYear':" + yop + "}",
    //    contentType: "application/json; charset=utf-8",
    //    dataType: 'json',
    //    success: function (response) {
    //        var maxmark = JSON.stringify(response.d);
    //        // $("#txtMaxMark").attr("readonly", true);
    //        $("#txtMaxMark").val(maxmark);
