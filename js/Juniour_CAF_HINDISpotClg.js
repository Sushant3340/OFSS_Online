...
function updateRow(ctlId) {
    ...
    if (totRow < 21) {
        ...
        if (totRow > 0) {
            ...
            //alert(totRow);
            if (parseInt(totRow) > 2) //&& (parseInt(stCntr) > 0))
            {

                alert('You cannot add more than 1 option');
                //  clearDDL();
            }
            else {
...
function ValidateForm() {
    ...
    if (((board == 46) && (yop >= 2010) && (yop <= 2017)) || ((board == 103) && (yop >= 2012))) {
        ...
        if (Tot >= Max) {
            alert('Total Mark obtained cannot be greater than equal to Total Full Marks');
            document.getElementById('txtTotMark').focus();
            return false;
        }
        if (Max == 0) {
        ...
        if (perc >= 100.00) {
            alert('Please enter valid total obtained marks');
            document.getElementById('txtTotMark').focus();
            return false;
        }
    ...
    else if ((document.getElementById('tableOption').getElementsByTagName("TR").length == 2) && document.getElementById('ddlStream').value != '0') {
        alert('You cannot add more than 1 option');
        return false;
    }
    ...
    if ((document.getElementById('rbtnMinority').checked == false) &&
        (document.getElementById('rbtnCoaching').checked == false) &&
        (document.getElementById('rbtnOtherQuota').checked == false)) {
...
//====================Load District =============    
function loadDistricts(intDistrictID) {
    debugger;
    ...
    $.ajax({
        type: 'POST',
        url: 'JrCAFForm.aspx/LoadDistrictClg',
        data: "{'intDistrictID':'" + intDistrictID + "'}",
        contentType: "application/json; charset=utf-8",
        success: function (response) {
            var Option; //= $('<option value="0">--SELECT--</option>');
            ...
            $.each(JSON.parse(JSON.stringify(response.d)), function (index, value) {
                Option = $('<option value="' + value.int_DistrictID + '">' + value.vch_DistrictName + '</option>');
                $('#ddlCollegeDistrict').append(Option);
                intDistrictIds = value.int_DistrictID;
            });
            loadColleges(intDistrictIds);
        },
        error: function (response) {
            var msg = jQuery.parseJSON(response.responseText);
            console.log("Message: " + msg.Message);
...
//=====================fill College===================
function loadColleges(intDistrictIds) {

    var inVal = intDistrictIds; // parseInt(document.getElementById('ddlCollegeDistrict').options[document.getElementById('ddlCollegeDistrict').selectedIndex].value);
    var intCollegeId;
    var selCollegeId = document.getElementById('hidCollegeID').value;
    if (inVal != '0') {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: 'JrCAFForm.aspx/fillDistWiseColgSelected',
            data: "{'intDistId':'" + inVal + "','intCType':'" + collegeType + "','intGender':'" + intGender + "','restype':'" + restype + "','CollegeId':'" + selCollegeId + "'}",
            contentType: "application/json; charset=utf-8",
            success: function (response) {
                var Option; // = $('<option value="0">--SELECT--</option>');
                ...
                $.each(JSON.parse(JSON.stringify(response.d)), function (index, value) {
                    ...
                    $('#ddlCollege').append(Option);
                    intCollegeId = value.CID;

                });
                fillStream(intCollegeId);
            },
            error: function (response) {
                var msg = jQuery.parseJSON(response.responseText);
                console.log("Message: " + msg.Message);
...
function fillStream(ctlDdlVal) {
    ...
    var inVal = ctlDdlVal;
    var appType=1;
    if (document.getElementById('rbtnMinority').checked) {
        appType = '1';
    }
    else if (document.getElementById('rbtnCoaching').checked) {
        appType = '2';
    }
    else {
        appType = '3';
    ...
    $.ajax({
        type: 'POST',
        url: 'JrCAFFormSpotClg.aspx/FillStream',
        data: "{'intCollegeID':'" + inVal + "','quotaType':'" + appType + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (response) {
            //  alert(response);
...
function VerifyMarks() {
    ...
    if (((board == 46) && (yop >= 2010) && (yop <= 2017)) || ((board == 103) && (yop >= 2012))) {
        ...
        msg = 'Total Full Marks - ' + $("#txtMaxMark").val() + ', Total Secured Marks -' + $("#txtTotMark").val();
    }
    return msg;
}
