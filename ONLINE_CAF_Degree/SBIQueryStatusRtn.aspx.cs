...
public partial class StudentLogin_SBIQueryStatusRtn : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Util.LogError(ex, "SBIPaymentUpdate_Deg");
        }
        finally
        {
    ...
    private void GetSBIResponse()
    {
        ...
        {
            Response.Redirect("UpdatePayment_Deg.aspx?sbi_St=3");
        }
        else
        {
            /*
            ...
            {
                Session["SBI_Id"] = objSbi.MerchantOrderNo;
                if (string.Equals(objSbi.Status, "success", StringComparison.OrdinalIgnoreCase))
                {
                    Response.Redirect("UpdatePayment_Deg.aspx?sbi_St=1");
                }
                else
                {
                    Response.Redirect("UpdatePayment_Deg.aspx?sbi_St=2");
                }
            }
            else
            {
